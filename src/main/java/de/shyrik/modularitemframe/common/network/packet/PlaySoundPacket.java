...
public class PlaySoundPacket {
    ...
    public static void handle(PlaySoundPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            SoundEvent sound = ForgeRegistries.SOUND_EVENTS.getValue(msg.soundId);
            Optional<SoundCategory> cat = Arrays.stream(SoundCategory.values()).filter(sc -> sc.getName().equals(msg.soundCategory)).findFirst();
            if (sound != null && cat.isPresent())
                mc.getSoundHandler().play(new SimpleSound(sound, cat.get(), msg.volume, msg.pitch, msg.pos));
        });
