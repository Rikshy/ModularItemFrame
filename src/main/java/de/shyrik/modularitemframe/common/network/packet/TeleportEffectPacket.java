...
public class TeleportEffectPacket  {
    ...
    public static void handle(TeleportEffectPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            for (int i = 0; i < 128; i++) {
                mc.world.addParticle(Particles.PORTAL, msg.pos.getX() + (mc.world.rand.nextDouble() - 0.5) * 3, msg.pos.getY() + mc.world.rand.nextDouble() * 3, msg.pos.getZ() + (mc.world.rand.nextDouble() - 0.5) * 3, (mc.world.rand.nextDouble() - 0.5) * 2, -mc.world.rand.nextDouble(), (mc.world.rand.nextDouble() - 0.5) * 2);
            }
        });
    }
}
